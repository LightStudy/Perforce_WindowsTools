"""
:author: LightStudy 
:contact: TOBEALISTENER@outlook.com
:file: mainGUI.py
:time: 2021/10/9 0:26
:@Software: Visual Studio Code 
:desc: 该文件完成了主窗体逻辑设计，软件运行时应在此初始化和调用其他窗体。 
"""
...
class MainWindow(tk.Tk):
    def __init__(self):
        ...
        self.btn_query = tk.Button(
            self.frame_top, text="查询", command=self.query)
        ...
        self.btn_delete = tk.Button(
            self.frame_bottom, text="删除", command=self.delete)
        self.btn_update = tk.Button(
            self.frame_bottom, text="修改", command=self.update)
        self.btn_add = tk.Button(
            self.frame_bottom, text="添加", command=self.add)
        ...
        self.tree = ttk.Treeview(
            self.frame_center, show="headings", height=8, columns=("a", "b", "c", "d"))
...
class MyDialog(tk.Toplevel):
    ...
    def setup_ui(self):
        ...
        tk.Label(row1, textvariable=self.text_error_no,
                 width=20, fg='red').pack(side=tk.LEFT)
        # 第二行
        row2 = tk.Frame(self)
        ...
        tk.Label(row2, textvariable=self.text_error_name,
                 width=20, fg='red').pack(side=tk.LEFT)
        # 第三行
        row3 = tk.Frame(self)
        ...
        tk.Label(row3, textvariable=self.text_error_pname,
                 width=20, fg='red').pack(side=tk.LEFT)
        # 第四行
        row4 = tk.Frame(self)
        ...
        tk.Label(row4, textvariable=self.text_error_pnum,
                 width=20, fg='red').pack(side=tk.LEFT)
        # 第五行
        row5 = tk.Frame(self)
        row5.grid(row=4, column=0, padx=160, pady=20)
        tk.Button(row5, text="取消", command=self.cancel).grid(
            row=0, column=0, padx=60)
        tk.Button(row5, text="确定", command=self.ok).grid(
            row=0, column=1, padx=60)
    ...
    def ok(self):
        self.group_info = [self.no.get(), self.name.get(
        ), self.pname.get(), self.pnum.get()]  # 设置数据
